...
type Route = "today" | "history" | "backtest" | "method";
const NAV: [Route,string,string][] = [["today","今日信号","⌁"],["history","历史记录","◷"],["backtest","策略回测","↗"],["method","方法说明","◇"]];
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="main">
      {error ? <ErrorState message={error}/> : !snapshot ? <Loading/> : route==="today" ? <Today snapshot={snapshot}/> : route==="history" ? <History snapshot={snapshot} manifest={manifest}/> : route==="backtest" ? <Backtest snapshot={snapshot}/> : <Method snapshot={snapshot}/>} 
    </main>
  </div>
}
...
function OutcomeCell({horizon,outcome}:{horizon:number,outcome?:CandidateOutcome}) { if(!outcome)return <div className="outcome-cell pending"><small>T+{horizon}</small><b>观察中</b></div>; const value=outcome.net_return; return <div className="outcome-cell"><small>T+{horizon} · {outcome.exit_date}</small><b className={value>0?"return-up":value<0?"return-down":""}>{signedPct(value)}</b><span>最大浮亏 {pct(Math.min(outcome.mae,0))}</span></div> }
