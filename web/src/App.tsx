...
function Today({snapshot}:{snapshot:Snapshot}) {
  ...
  return <div className="content"><PageHeader eyebrow="DAILY SIGNALS" title="今日技术面候选"><div className="date-pill"><span/>截至 {snapshot.as_of_date} 收盘</div></PageHeader>
    <section className="hero"><div><span className="hero-kicker">今日信号摘要</span><h2>从完整自选池中，筛出<br/><em>{snapshot.summary.top10_count} 只</em>值得优先研究的候选</h2><p>优先呈现基本面已有研究基础，并出现底背离、确认金叉或潜在金叉共振的股票。</p></div><div className="hero-ring"><strong>{(snapshot.coverage.coverage_ratio*100).toFixed(1)}%</strong><span>数据覆盖率</span><small>{snapshot.coverage.covered_count} / {snapshot.coverage.universe_count}</small></div></section>
